...
- Exposed the runtime snapshot from `main.py` without starting any new
  platform service or changing existing Windows behavior.

## Deliberate limits

- Services remain unwired; only the immutable capability snapshot is exposed
  from `main.py`.
