...
from core.voice.performance_governor import VoicePerformanceGovernor
...
class VoiceService:
    # Phase 40: gobernador adaptativo de recursos + menor ruido de UI
    # Phase 39: gobernador de recursos + warmup de modelo fondo
    ...
    def __init__(self, wiz) -> None:
        ...
        self._governor = VoicePerformanceGovernor()  # Phase 40: gobernador adaptativo
        self._warmup_thread: threading.Thread | None = None  # Phase 39: warmup ASR en segundo plano
    ...
    def apply_performance_profile(self, profile: str) -> dict[str, Any]:
        ...
        payload["adaptive_governor_enabled"] = True
        payload.setdefault("continuous_emit_transcribing_events", False)
        payload.setdefault("continuous_emit_idle_events", False)
        ...
        payload.setdefault("continuous_audio_block_ms", 40)
        payload.setdefault("continuous_idle_cpu_saver_enabled", False)
    ...
    def _capture(self, *, continuous: bool = False) -> AudioCaptureResult:
        ...
        if bool(cfg.get("smart_recording", True)):
            ...
            return audio.record_smart_to_wav(
                ...
                block_ms=int(cfg.get("continuous_audio_block_ms", 40) if continuous else cfg.get("audio_block_ms", 40)),
    ...
    def _capture_wake_probe(self) -> AudioCaptureResult:
        ...
        return self._capture_custom(
            ...
            min_seconds=float(overrides.get("min_speech", cfg.get("wake_probe_min_speech_seconds", 0.30))),
    ...
    def _emit(self, payload: dict[str, Any]) -> None:
        # Phase 40: no refrescar UI por cada microestado si no aporta.
        # El motor sigue trabajando igual; solo baja ruido/costo visual.
        try:
            event = str(payload.get("event") or "")
            cfg = self.config()
            if event == "transcribing" and not bool(cfg.get("continuous_emit_transcribing_events", False)):
                return
            if event == "idle" and not bool(cfg.get("continuous_emit_idle_events", False)):
                return
            if bool(cfg.get("voice_governor_diagnostics", False)):
                payload.setdefault("governor", self._governor.snapshot())
        except Exception:
            pass
    ...
    def _non_wake_pause_seconds(self) -> float:
        # Phase 40: pausa adaptativa. Rápida cuando importa; más tranquila
        # cuando hay conversaciones/ruido sin activador.
        try:
            return self._governor.non_wake_pause_seconds(self.config())
        except Exception:
            cfg = self.config()
    ...
    def _register_non_wake(self) -> None:
        ...
        try:
            self._governor.observe_non_wake()
        except Exception:
            pass
    ...
    def _continuous_loop_wake_then_command(self, cfg: dict[str, Any], started: float) -> None:
        ...
        if not capture.speech_started:
            try:
                self._governor.observe_silence()
            except Exception:
                pass
            ...
            time.sleep(self._governor.idle_pause_seconds(cfg))
            return
        ...
        min_transcribe = float(cfg.get("wake_min_transcribe_seconds", 0.48))
        short_candidate = (
        ...
        if self._looks_like_hallucination(wake_text, capture_seconds=float(capture.seconds)):
            try:
                self._governor.observe_hallucination()
            except Exception:
                pass
        ...
        command_text = self._sanitize_continuous_command_text(command_text, "continuous_command")
        ...
        self._emit(result)
        self._governor.observe_command(result.get("elapsed")) if result.get("ok") else None
        time.sleep(self._governor.after_command_pause_seconds(cfg, bool(result.get("ok"))))
    ...
    def _handle_candidate_wake_without_asr(self, cfg: dict[str, Any], started: float, capture: AudioCaptureResult, *, reason: str) -> None:
        ...
        command_text = self._sanitize_continuous_command_text(command_text, "continuous_candidate")
    ...
    def _detect_wake_prefix(self, text: str) -> tuple[bool, str, str | None]:
        ...
        for word in aliases:
            ...
            if norm.startswith(word + " "):
                remainder = norm[len(word):].strip()
                # Phase 45: si Whisper devuelve "pc pc ...", no dejar el segundo
                # activador dentro del comando porque puede disparar fuzzy matches.
                parts = remainder.split()
                while parts and parts[0] in alias_set:
                    parts.pop(0)
    ...
    # ------------------------------------------------------------------ #
    # Phase 45: guardas anti-falsos positivos en escucha continua
    # ------------------------------------------------------------------ #
    def _sanitize_continuous_command_text(self, text: str, source: str) -> str:
        """Limpia activadores repetidos que Whisper puede meter en el comando.

        Ejemplo real: "pc, pc, sado" -> "sado". Luego el guard lo rechaza
        por ser un color ambiguo sin verbo/intent claro.
        """
        if source not in ("continuous", "continuous_command", "continuous_candidate"):
            return text
        if not bool(self.config().get("continuous_strip_repeated_wake_words", True)):
            return text
    ...
    def _continuous_has_action_word(self, text: str) -> bool:
        words = set(normalize_text(text).split())
        action_words = {
            # ES
            "pon", "pone", "poner", "ponle", "dejalo", "dejala", "deja", "dejar",
            "cambia", "cambiar", "activa", "activar", "modo", "brillo", "luz", "luces",
            "prende", "prender", "enciende", "encender", "apaga", "apagar", "sube", "subir",
            "baja", "bajar", "alterna", "alternar", "toggle", "calido", "frio", "blanco",
            # EN
            "set", "turn", "switch", "on", "off", "brightness", "color", "mode", "movie",
        }
        return bool(words & action_words)

    def _continuous_is_color_action(self, intent) -> bool:
        try:
            blob = str(getattr(intent, "action", None) or "").lower()
        except Exception:
            blob = ""
        color_tokens = (
            "rgb", "color", "rojo", "rosado", "rosa", "azul", "verde", "amarillo", "naranjo", "naranja",
            "morado", "violeta", "lila", "fucsia", "magenta", "cian", "celeste", "turquesa", "calipso",
            "red", "blue", "green", "yellow", "orange", "purple", "pink", "cyan",
        )
        return any(tok in blob for tok in color_tokens)

    def _continuous_execution_guard(self, parsed_text: str, intent, source: str) -> dict[str, object] | None:
        """Evita que conversaciones normales ejecuten acciones por fuzzy matching.

        No afecta push-to-talk ni pruebas de texto manuales. Solo se aplica a
        escucha continua, donde los falsos positivos son más peligrosos.
        """
        if source not in ("continuous", "continuous_command", "continuous_candidate"):
            return None
        cfg = self.config()
        norm = normalize_text(parsed_text)
        words = norm.split()
        if not words:
            return {
        ...
        one_word_ok = {"apaga", "apagar", "prende", "prender", "enciende", "encender", "toggle", "alterna", "on", "off"}
        if len(words) == 1 and words[0] not in one_word_ok:
            return {
                "ok": False, "source": source, "transcript": parsed_text, "confidence": float(getattr(intent, "confidence", 0.0) or 0.0),
                "message": "Ignorado: comando demasiado corto/ambiguo en escucha continua", "action": getattr(intent, "action", None),
                "intent_source": "continuous_safety",
            }

        min_conf = float(cfg.get("continuous_min_execute_confidence", 0.78) or 0.78)
        conf = float(getattr(intent, "confidence", 0.0) or 0.0)
        if conf and conf < min_conf:
            return {
                "ok": False, "source": source, "transcript": parsed_text, "confidence": conf,
                "message": f"Ignorado: baja confianza en segundo plano ({conf:.2f})", "action": getattr(intent, "action", None),
                "intent_source": "continuous_safety",
            }

        # Colores por fuzzy matching son el falso positivo más común.
        # Ej: "pc, pc, sado" -> parser lo puede mapear a "Rosado".
        if bool(cfg.get("continuous_color_requires_action_word", True)) and self._continuous_is_color_action(intent):
            if not self._continuous_has_action_word(norm):
                return {
                    "ok": False, "source": source, "transcript": parsed_text, "confidence": conf,
                    "message": "Ignorado: color ambiguo; di 'pc pon rosado' o 'pc color rosado'", "action": getattr(intent, "action", None),
                    "intent_source": "continuous_safety",
                }
        return None
    ...
    def execute_text(self, text: str, source: str = "manual") -> dict[str, Any]:
        ...
        parsed_text = self._sanitize_continuous_command_text(parsed_text, source)
        intent = self.parser.parse(parsed_text)
        safety = self._continuous_execution_guard(parsed_text, intent, source)
        if safety:
            return safety
